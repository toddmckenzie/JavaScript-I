...
for (let i = 0; i < inventory.length; i++) {
    if (inventory[i]["id"] === 33) {
      console.log('Challenge 1. ', inventory[i]["car_year"], inventory[i]["car_make"], inventory[i]["car_model"] )
  }
};
...
console.log('Challenge 2. ', inventory[lastCar]["car_make"], inventory[lastCar]["car_year"] )
...
console.log('Challenge 3. ', carModels.sort());
...
console.log('Challenge 4. ', carYears.sort());

// ==== Challenge 5 ====
// The car lot manager needs to find out how many cars are older than the year 2000. Using the carYears array you just created, find out how many cars were made before the year 2000 by populating the array oldCars and logging it's length.
let oldCars = [];
inventory.filter(car => {
  (car["car_year"] < 2000) ? oldCars.push(car): null;
});
console.log('Challenge 5. ', oldCars.length);

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi = [];
inventory.filter(car => {
  (car["car_make"] === 'BMW' || car["car_make"] === 'Audi') ? BMWAndAudi.push(car): null;
})
console.log('Challenge 6. ', JSON.stringify(BMWAndAudi));
